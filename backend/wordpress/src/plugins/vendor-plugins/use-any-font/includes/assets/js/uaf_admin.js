function uaf_lite_api_key_generate() {
  jQuery.ajax({
    url: uaf_server_url + "/uaf_convertor/generate_lite_key.php",
    beforeSend: function () {
  ...
  jQuery("#open_add_font_form_js").submit(function (e) {
    ...
    jQuery.ajax({
      ...
      beforeSend: function () {
        ...
        jQuery("#font_upload_message").html(
          "Uploading Font. It might take few mins based on your font file size."
        );
      },
      success: function (data, textStatus, jqXHR) {
        ...
        if (status == "error") {
          ...
          if (woffStatus == "ok" && woff2Status == "ok") {
            ...
            jQuery("#font_upload_message").attr("class", "ok");
            jQuery("#font_upload_message").html(
              "Font Conversion Complete. Finalizing..."
            );
            ...
            jQuery("#font_upload_message").html(
              "Problem converting font to woff/woff2 formats. Please contact support."
            );
          }
        }
      },
      error: function (jqXHR, textStatus, errorThrown) {
        jQuery("#font_upload_message").attr("class", "error");
        jQuery("#font_upload_message").html(
          "Unexpected Error Occured. Please set Use PHP Uploader to Yes from Settings Tab and try again."
        );
        jQuery("#submit-uaf-font").attr("disabled", false);
      },
    });
